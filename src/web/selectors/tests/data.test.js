...
describe('selectors', () => {
  describe('data', () => {
    const countryDimension = {
      ...
      index: 0,
      label: 'Countries',
      values: [{ id: 'au', label: 'Australia' }, { id: 'fr', label: 'France', isSelected: true }],
    };
    const otherDimensions = [
      {
        ...
        index: 1,
        label: 'Sex',
        values: [{ id: 'm', label: 'Male' }, { id: 'f', label: 'Female', isSelected: true }],
      },
      {
        id: 'TYPE',
        index: 2,
    ...
    it('should getIsLoadingStructure', () => {
      expect(selectors.getIsLoadingStructure(state)).toEqual(state.data.isLoadingStructure);
    });
